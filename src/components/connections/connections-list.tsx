import type { ConnectionSummary } from "#/server/db/queries/connections";

const dateFormatter = new Intl.DateTimeFormat("fr-FR", { dateStyle: "medium" });

const BUSINESS_MANAGE_SCOPE = "https://www.googleapis.com/auth/business.manage";

export function ConnectionsList({
	connections,
}: {
	connections: readonly ConnectionSummary[];
}) {
	if (connections.length === 0) return null;
	return (
		<ul className="divide-y divide-neutral-200 overflow-hidden rounded-md border border-neutral-200 bg-white">
			{connections.map((c) => (
				<ConnectionRow key={c.id} connection={c} />
			))}
		</ul>
	);
}

function ConnectionRow({ connection }: { connection: ConnectionSummary }) {
	return (
		<li className="flex items-center justify-between gap-4 p-4">
			<div className="flex min-w-0 items-center gap-3">
				<PlatformLogo platform={connection.platform} />
				<div className="min-w-0">
					<div className="font-medium text-neutral-900 text-sm">
						{platformLabel(connection.platform)}
					</div>
					<div className="truncate text-neutral-500 text-xs">
						{connection.platformAccountLabel ?? "Compte sans email"}
						{" · Connecté le "}
						{dateFormatter.format(new Date(connection.createdAt))}
					</div>
				</div>
			</div>
			<ConnectionStatus connection={connection} />
		</li>
	);
}

function ConnectionStatus({ connection }: { connection: ConnectionSummary }) {
	if (connection.lastSyncError) {
		return (
			<span className="rounded-full bg-red-100 px-2 py-0.5 font-medium text-red-800 text-xs">
				Erreur de sync
			</span>
		);
	}
	// Google grants scopes incrementally: while the Business Profile API
	// access request is pending, `business.manage` is silently dropped even
	// though we request it. Surface this so the user understands the state.
	if (
		connection.platform === "google" &&
		!connection.scopes?.includes(BUSINESS_MANAGE_SCOPE)
	) {
		return (
			<span
				title="Le scope business.manage n'a pas été accordé — reconnexion nécessaire une fois Google Business Profile API approuvée."
				className="rounded-full bg-amber-100 px-2 py-0.5 font-medium text-amber-800 text-xs"
			>
				Accès limité
			</span>
		);
	}
	return (
		<span className="rounded-full bg-emerald-100 px-2 py-0.5 font-medium text-emerald-800 text-xs">
			Actif
		</span>
	);
}

function platformLabel(platform: ConnectionSummary["platform"]): string {
	switch (platform) {
		case "google":
			return "Google Business Profile";
		case "tripadvisor":
			return "TripAdvisor";
		case "trustpilot":
			return "Trustpilot";
		case "thefork":
			return "TheFork";
	}
}

function PlatformLogo({
	platform,
}: {
	platform: ConnectionSummary["platform"];
}) {
	if (platform === "google") return <GoogleLogo />;
	return (
		<div
			aria-hidden="true"
			className="size-6 rounded bg-neutral-100"
			role="presentation"
		/>
	);
}

function GoogleLogo() {
	return (
		<svg
			viewBox="0 0 24 24"
			className="size-6 shrink-0"
			aria-hidden="true"
			role="presentation"
		>
			<title>Google</title>
			<path
				d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
				fill="#4285F4"
			/>
			<path
				d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
				fill="#34A853"
			/>
			<path
				d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
				fill="#FBBC05"
			/>
			<path
				d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
				fill="#EA4335"
			/>
		</svg>
	);
}
